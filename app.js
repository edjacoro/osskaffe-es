...
function scheduleSharedStateSave() {
  if (!sharedStateEnabled || suppressSharedStateSave) return;
...
async function connectSharedState(role, employeeId = null) {
  try {
    ...
    if (loginResponse.status === 404) return { available: false, authenticated: true };
    if (!loginResponse.ok) return { available: true, authenticated: false };
    ...
    return { available: false, authenticated: true };
  }
}
...
async function refreshSharedState() {
  ...
  try {
    ...
    state = seedDefaultHolidays(mergeState(DEFAULT_STATE, payload.state));
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    if (appRole === 'admin' && adminInited) render();
...
async function setEmployeeMode(employeeId) {
  await connectSharedState('employee', employeeId);
  appRole = "employee";
